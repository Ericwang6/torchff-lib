...
def test_md():
    cutoff = 1.2
    water_data = get_water_data(10000, cutoff+0.2, torch.float32, 'cuda', False, False, False)
    coords_start = water_data.coords.clone()
    # print(water_data.forces)
    ...
    with torch.cuda.graph(g):
        velocities.add_(forces / water_data.mass * dt / 2)
