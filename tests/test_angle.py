...
def test_perf_harmonic_angle(device, dtype):
    ...
    angles = torch.tensor([random.sample(arange, 3) for _ in range(Nangles)], device=device)
    coords = torch.rand(N*3, 3, requires_grad=requires_grad, device=device, dtype=dtype)
    theta0 = torch.rand(Nangles, device=device, dtype=dtype, requires_grad=False)
    k = torch.rand(Nangles, device=device, dtype=dtype, requires_grad=False)

    func = HarmonicAngle(use_customized_ops=True)
    func_ref = torch.compile(HarmonicAngle(use_customized_ops=False))
