...
def plot_results(results: List[Dict[str, Any]], pdf_path: str) -> None:
    ...
    """
    ranks = [0, 1, 2]
    impl_labels = {"ref": "Naive Python", "custom": "TorchFF"}
    impl_styles = {"ref": "o-", "custom": "s-"}
...
def main() -> None:
    ...
    system_sizes = [100, 300, 1000, 3000, 10000]
    ranks = [0, 1, 2]
    ...
    for rank in ranks:
        for num_atoms in system_sizes:
            ...
            try:
                ...
                torch.cuda.empty_cache()
                # Record failed entries for both implementations
                for impl in ("ref", "custom"):
                    all_results.append(
                        {
                            "rank": rank,
